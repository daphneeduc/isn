...
function ready() {
    var supprimerCartItemButtons = document.getElementsByClassName('btn-danger') 
    //création d'un bouton pour supprimer chaque produit du panier
    for (var i = 0; i < supprimerCartItemButtons.length; i++) { 
        //permet de faire un bouton pour tous les produits du panier
        var button = supprimerCartItemButtons[i]
        button.addEventListener('click', supprimerCartItem) 
        //permet qu'à chaque click sur le bouton l'action supprimer s'éxécute
    } //"addEventListener"est lié à une fonction et va exécuter la fonction l.46

    var quantityInputs = document.getElementsByClassName('cart-quantity-input')
    //on veut que le prix en fonction de la quantité s'actualise dans le panier
    for (var i = 0; i < quantityInputs.length; i++) { //la quantité ne peut pas être négative
        ...
        //"addEventListener"est lié à une fonction va s'exécuter à chaque fois que la quantité change l.52
    }

    var addToCartButtons = document.getElementsByClassName('shop-item-button')
    //variable pour ajouter au panier liée aux élements "shop-item-button" dans la page html
    for (var i = 0; i < addToCartButtons.length; i++) { //même principe que pour les autres boutons
        var button = addToCartButtons[i]
        button.addEventListener('click', addToCartClicked)
        //quand l'utilisateur click sur le bouton on renvoit à une fonction l.62
    }

    document.getElementsByClassName('btn-purchase')[0].addEventListener('click', purchaseClicked) 
    //on crée un bouton pour le bouton ACHETER l.37
}

function purchaseClicked() { //fonction du bouton "ACHETER" 
  alert('Merci pour votre achat') //envoie un message d'alerte quand on clique sur le bouton
...
function supprimerCartItem(event) { //fonction du bouton supprimer
    ...
    //permet de supprimer tout l'element "parent", c'est à dire toute la division
...
function addToCartClicked(event) {
    ...
    //faire afficher le titre de l'article dans le panier
    var price = shopItem.getElementsByClassName('shop-item-price')[0].innerText 
    //faire afficher le prix de l'article dans le panier
    var imageSrc = shopItem.getElementsByClassName('shop-item-image')[0].src 
    //faire afficher l'image de l'article dans le panier
    var id = shopItem.dataset.itemId
    var taille = shopItem.getElementsByClassName("shop-item-size")[0].firstElementChild.value;
    //on récupère le premier element enfant qui est le noeud option puis on récupère la valeur
...
function addItemToCart(title, price, imageSrc, id, taille) { //fonction ajouter au panier
 ...
 //on veut que chaque article qui se rajoute au panier,pas seulement le premier, se rajoute en dessous du premier article ajouté
  //on prend en compte tous les élements que l'on souhaite afficher :
    ...
    cartRow.getElementsByClassName('btn-danger')[0].addEventListener('click', supprimerCartItem) 
    //sans mettre ce code, le bouton "supprimer" ne marche pas donc de la même manière que l.11
    cartRow.getElementsByClassName('cart-quantity-input')[0].addEventListener('change', quantityChanged) 
    //faire afficher la quantité pour chaque article même principe que l.17
}

function updateCartTotal() {
    //on prend en compte toutes les variables de chaque rangées(donc chaque article) pour savoir le prix et estimer le total
    var cartItemContainer = document.getElementsByClassName('cart-items')[0] 
    //on appelle la section de la page html pour trouver les élements suivants
    ...
    for (var i = 0; i < cartRows.length; i++) { // prend en compte toutes les rangées donc tous les articles
        ...
        // prend en compte le prix de chaque article dans chaque rangée
        var quantityElement = cartRow.getElementsByClassName('cart-quantity-input')[0] 
        //prend en compte la quantité demandée pour chaque article, chaque rangée
        var price = parseFloat(priceElement.innerText.replace('€', '')) 
        // pour faire un calcul il faut enlever le signe € pour obtenir un nombre
        //"parseFloat" permet de prendre en compte le nombre en nombre décimal pour faire le calcul
        var quantity = quantityElement.value
        //on attribue la variable de la quantité à la valeur de l'élement quantité dans la rangée
    ...
    //permet de faire afficher le résultat du calcul
}
